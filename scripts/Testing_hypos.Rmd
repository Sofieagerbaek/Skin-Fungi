...
Kruskal–Wallis test, is at least one family different in each category
...
pairwise
```{r}
pw_secr_test <- emmeans(model, pairwise ~ species, type = "response", adjust = "BH")$contrasts
pw_secr_test_df <- as.data.frame(pw_secr_test)
pw_secr_test_df$sig <- pw_secr_test_df$p.value < 0.05

ci_pw <- confint(pw_secr_test)

pw_secr_test_df <- pw_secr_test_df %>%
  left_join(ci_pw %>% select(contrast, asymp.LCL, asymp.UCL), by = "contrast") %>%
  rename(
    CI_lower = asymp.LCL,
    CI_upper = asymp.UCL
  )

pw_secr_test_df$sample1 <- str_split_fixed(pw_secr_test_df$contrast, " / ", 2)[, 1]
pw_secr_test_df$sample2 <- str_split_fixed(pw_secr_test_df$contrast, " / ", 2)[, 2]
pw_secr_test_df <- pw_secr_test_df %>% select(-c("contrast", "null", "df", "z.ratio"))

t <- pw_secr_test_df %>% filter(sig == "TRUE") 
head(t)
table(rbind(t$sample1, t$sample2))

pw_secr_test_df <- pw_secr_test_df %>%
  select(7, 8, everything())

#write.csv(pw_secr_test_df, "../generated/pw_sp_secreted_test.csv", row.names = FALSE)
```
...
res_carb <- df_carbs %>%
  group_by(category, desc_collapse) %>%
  group_modify(~{
    m <- glm.nb(
...
long_carb$sig <- long_carb$p.value < 0.05
#write.csv(long_carb, "../generated/secr_carb_pw.csv", row.names = FALSE)
...
#write.csv(long_lipid, "../generated/secr_lipid_pw.csv", row.names = FALSE)
...
#write.csv(long_protein, "../generated/secr_protein_pw.csv", row.names = FALSE)
...
#write.csv(full_res, "../generated/secreted_cat_anova.csv", row.names = FALSE)
```
...
## Virulence
We use Fishers exact to test presence/abscence of putative virulence-associated genes between non-pathogenic outgroups and the skin pathobionts

```{r}
test_cur <- read_xlsx("../generated/1211_manual_cur_secreted.xlsx")

virulence_assoc <- test_cur %>% filter(category == "virulence associated") %>%
...
comb_viru <- rbind(viru_non_secr, virulence_assoc) %>% filter(!category == "adhesin-associated")
...
```{r}
viru_df_fish_test <- comb_viru %>% select(-c("number_of_species","signalp","tmhmm_geneswtmh",
                                             "netgpi_geneswithgpi","number_of_genera",
                                             "number_of_families", "pfam_names", "category",
                                             "total_genec", "secr"))
species_cols <- names(viru_df_fish_test)[3:68]

#presence absence
vir_binary <- viru_df_fish_test
vir_binary[, species_cols] <- (vir_binary[, species_cols] > 0) * 1
```
...
Malassezia vs. Rhodotorula

```{r}
pathogen_species <- grep("^(Mala)", 
                      colnames(viru_df_fish_test), 
                      value = TRUE)
outgroup_species <- "Rhodotorula_toruloides_Z1"

a <- sum(vir_binary[, pathogen_species])
b <- length(pathogen_species) * nrow(vir_binary) - a

c <- sum(vir_binary[, outgroup_species])
d <- nrow(vir_binary) - c
...
enrich <- ifelse(ft$estimate > 1, "Group1 enriched", "Group2 enriched")

# Adding results as a new row to the df
fisher_results <- rbind(fisher_results, data.frame(
  clade = "Malassezia vs R. toruloides",
  group1 = "Malassezia spp.",
  group2 = "R. toruloides",
...
# Adding results as a new row to the df
fisher_results <- rbind(fisher_results, data.frame(
  clade = "Trichosporon ovoides vs R. toruloides",
  group1 = "T. ovoides",
  group2 = "R. toruloides",
  odds_ratio = as.numeric(ft$estimate),
  ci_lower = ft$conf.int[1],
...
Candida, Pichia, Nakaseomyces vs. saccharomyces
```{r}
pathogen_species <- grep("^(Candida|Nakaseomyces|Pichia)", 
                      colnames(viru_df_fish_test), 
                      value = TRUE)
outgroup_species <- "Saccharomyces_cerevisiae_wine010"
...
# Adding results as a new row to the df
fisher_results <- rbind(fisher_results, data.frame(
  clade = "Patho Saccharomycetales vs S. cerevisiae",
  group1 = "Candida spp., Pichia, Nakaseomyces",
  group2 = "S. cerevisiae",
  odds_ratio = as.numeric(ft$estimate),
  ci_lower = ft$conf.int[1],
  ci_upper = ft$conf.int[2],
...
Dermatophytes
```{r}
pathogen_species <- grep("^(Trichop|Epi|Nann|Micro)", 
                      colnames(viru_df_fish_test), 
                      value = TRUE)
outgroup_species <- "Chrysosporium_queenslandicum_28077"
...
# Adding results as a new row to the df
fisher_results <- rbind(fisher_results, data.frame(
  clade = "Dermatophytes vs. C. queenslandicum",
  group1 = "Dermatophytes",
  group2 = "C. queenslandicum",
  odds_ratio = as.numeric(ft$estimate),
  ci_lower = ft$conf.int[1],
  ci_upper = ft$conf.int[2],
  p_value = ft$p.value,
...
# Adding results as a new row to the df
fisher_results <- rbind(fisher_results, data.frame(
  clade = "Hortaea werneckii vs. C. queenslandicum",
  group1 = "H. werneckii",
  group2 = "C. queenslandicum",
  odds_ratio = as.numeric(ft$estimate),
  ci_lower = ft$conf.int[1],
...
### potential: species within same genus, which are potentially differently virulent

Candida albicans vs. rest of Candida 
```{r}
pathogen_species <- grep("^(Candida_a)", 
                      colnames(viru_df_fish_test), 
                      value = TRUE)

outgroup_species <- grep("^(Candida_m|Candida_o|Candida_p|Candida_d|Candida_t)", 
                      colnames(viru_df_fish_test), 
                      value = TRUE)
...
c <- sum(vir_binary[, outgroup_species])
d <- length(outgroup_species) * nrow(vir_binary) - c
...
# Adding results as a new row to the df
fisher_results <- rbind(fisher_results, data.frame(
  clade = "C.albicans vs. other Candida spp.",
  group1 = "C. albicans",
  group2 = "other Candida spp.",
  odds_ratio = as.numeric(ft$estimate),
  ci_lower = ft$conf.int[1],
  ci_upper = ft$conf.int[2],
  p_value = ft$p.value,
...
M. canis vs. other Microsporum spp.
```{r}
pathogen_species <- grep("^(Microsporum_c)", 
                      colnames(viru_df_fish_test), 
                      value = TRUE)

outgroup_species <- grep("^(Microsporum_f|Microsporum_a)", 
                      colnames(viru_df_fish_test), 
                      value = TRUE)
...
# Adding results as a new row to the df
fisher_results <- rbind(fisher_results, data.frame(
  clade = "M. canis vs. other Microsporum spp.",
  group1 = "M. canis",
  group2 = "Other Microsporum spp.",
  odds_ratio = as.numeric(ft$estimate),
  ci_lower = ft$conf.int[1],
  ci_upper = ft$conf.int[2],
  p_value = ft$p.value,
...
T. indotineae vs. T. mentagrophytes & T. interdigitale
```{r}
pathogen_species <- grep("^(Trichophyton_ind)", 
                      colnames(viru_df_fish_test), 
                      value = TRUE)

outgroup_species <- grep("^(Trichophyton_m|Trichophyton_int)", 
                      colnames(viru_df_fish_test), 
                      value = TRUE)
...
# Adding results as a new row to the df
fisher_results <- rbind(fisher_results, data.frame(
  clade = "T. indotineae vs. T. ment & T. inter",
  group1 = "T. indotineae",
  group2 = "T. mentagrophytes & T. interdigitale",
  odds_ratio = as.numeric(ft$estimate),
  ci_lower = ft$conf.int[1],
  ci_upper = ft$conf.int[2],
  p_value = ft$p.value,
...
Save to file
```{r}
#write.csv(fisher_results, "../generated/viru_fishers_results.csv")
```
