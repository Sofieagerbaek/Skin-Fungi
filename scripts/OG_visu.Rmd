...
species  <- sapply(split_name_col, `[`, 2)
...
```{r}
sum((N0$Aspergillus.fumigatus_IP24 == ''))
...
og_groups <- og_groups %>%
  mutate(category = case_when(
    ...
    n_species == 1 ~ "Species",
  #  n_species == 1 & n_sample == 1 ~ "sample", # We include these in the species-specific for better visualisation
    TRUE ~ "Other"
  ))
```

Count OGs in each sample in each catgory
```{r}
...
```{r}
#library(ape)
#tree <- read.tree("../results/Results_Aug23/Species_Tree/SpeciesTree_rooted_node_labels.txt")
...
Making upset dataframe with presence/abscence for each genus in each OG
```{r}
...
upsetplot <- upset(upset_df, 
      intersect = intersect, # colnames(upset_df)[2:17], 
